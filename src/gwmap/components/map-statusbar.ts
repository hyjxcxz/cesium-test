...
let cesiumWidgetsFunc:any = null
let getElevationTimer:any = null
/**
 * 组件入口
 */
mapStatusBar.init = function (viewer:any) {
  if (!viewer) return
  // 三维地图
  if (!cesiumWidgetsFunc) {
    cesiumWidgetsFunc = new WindEarth.WidgetsFunc(viewer)
    cesiumWidgetsFunc.bindStatusInfoFunc((infos: { longitude: any; latitude: any; cameraHeight: any }) => {
      // 判断地球高度
      if (infos) {
 ...
 * @param {*} cameraHeight
 */
const updateStatus = function (lon:any, lat:any, cameraHeight:any) {
  if (getElevationTimer) {
    clearTimeout(getElevationTimer)
  }
  if (!lon && !lat && !cameraHeight) {
    mapStatusBar.element.innerHTML = ''
    mapStatusBar.element.style.display = 'none'
    return
  }
  mapStatusBar.element.style.display = 'inherit'
