...
const mastDatangLayer: any = {}
// let mastModule: any = null
let mastFeatureLayer: any = null

mastDatangLayer.loadMast = function (dataList: any, call: any) {
  mastDatangLayer.remove()
      ...
      setTimeout(() => {
        if (!res.feature || !res.position) {
          ...
          if (!String(res.feature.id).includes('_label')) {
            ...
            mastDatangLayer.zoomToFeature(res.feature._id)
          }
        }
      }, 100)
    })
...
mastDatangLayer.zoomToFeature = function (mastId:any, t:any, y:any) {
...
// 修改label属性
mastDatangLayer.changeProperty = (mast:any) => {
...
mastDatangLayer.remove = function () {
  if (!mastFeatureLayer) return
...
mastDatangLayer.mastLocationLayer = function () {
  mastFeatureLayer.locationLayer()
}
export default mastDatangLayer
...
function getLabel (item:any) {
  ...
  fillRoundRect(ctx, 49, 20, canvas.width / 2, canvas.height / 2, 10, 'rgba(83, 169, 255, 1)')

  strokeRoundRect(ctx, 44, 16, canvas.width / 2 + 10, canvas.height / 2 + 10, 14, 0, 0)
